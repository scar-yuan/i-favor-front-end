...
function TodoList() {
    ...
    useEffect(() => {
        ...
        axios.get('http://tqsmev.natappfree.cc/todo')
        .then((res) => {
    ...
    const add = (value) => {
        setUserTodo(userTodo => [...userTodo,{id:4,todo:value,done:false}])
        setFinshData((finshData) => {
            let newState = cloneDeep(finshData)
            newState.total +=1
            ...
            <div className="leftTodoBox" style={{display:"flex",flexDirection:"column",justifyContent:"space-between"}}>
                <div style={{display:"flex",justifyContent:"space-around",height:"10vh",alignItems:"center"}}>
                    ...
                    <div style={{position:"relative",width:"20vw",right:"-5vw"}}  >
                        <TodoInput  ref={inputRef} placeholder="      请输入Todo~" type="text" />
                        <EditOutlined onClick={() => {add(inputRef.current.value) ;inputRef.current.value = ''}} style={{position:"absolute",fontSize:"1.4rem",right:"-9vw",top:"0.2vh"}} />
                    </div>
                </div>
